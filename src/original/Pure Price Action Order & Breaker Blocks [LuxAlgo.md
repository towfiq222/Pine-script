...
method display(ob id, css, break_css) =>
    ...
    if id.breaker
        box.new(id.loc, id.top, id.break_loc, id.btm, css, bgcolor = css, xloc = xloc.bar_time)
...
if barstate.islast
    //Bullish
    if showBull > 0
        for i = 0 to math.min(showBull - 1, bullish_ob.size() - 1) by 1
            get_ob = bullish_ob.get(i)
            get_ob.display(bullCss, bullBreakCss)

    //Bearish
    if showBear > 0
        for i = 0 to math.min(showBear - 1, bearish_ob.size() - 1) by 1
