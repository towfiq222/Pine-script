...
msTextT = input.string('Enabled', '  Market Structure Labels', options = ['Enabled', 'Disabled'], group = msGroup, display = display)
msText = msTextT == 'Enabled'
...
obbUseBodyT = input.string('Enabled', '  Use Candle Body', options = ['Enabled', 'Disabled'], group = obbGroup, display = display)
obbUseBody = obbUseBodyT == 'Enabled'
...
lqTextT = input.string('Disabled', '  Liquidity Void Labels', options = ['Enabled', 'Disabled'], group = liqGrp, display = display)
lqText = lqTextT == 'Enabled'
...
method renderStructures(ICTMS this, isBullish, marketStructure, color, style, labelEnabled) =>
    ...
    if condition and not this.isCrossed
        ...
        if labelEnabled
            this.msLabel := box.new(this.lastIndex, this.lastPrice, bar.index, this.lastPrice, color(na), bgcolor = color(na), text = marketStructure.type == (isBullish ? -1 : 1) ? 'MSS' : 'BOS', text_size = size.tiny, text_halign = text.align_left, text_valign = isBullish ? text.align_bottom : text.align_top, text_color = color.new(color, 17))
            this.msLabel
...
field_0 = bar.low
field_1 = bar.low
if queryPatterns(bar.low, field_0[1], field_1[2], false)
    field_2 = bar.low
    field_3 = bar.index
    stLow.updatePattern(field_2[1], field_3[1])
...
field_4 = bar.high
field_5 = bar.high
if queryPatterns(bar.high, field_4[1], field_5[2], true)
    field_6 = bar.high
    field_7 = bar.index
    stHigh.updatePattern(field_6[1], field_7[1])
    field_8 = bar.index
    field_9 = bar.high
    field_10 = bar.high
    stHigh.lastLabel := label.new(field_8[1], field_9[1], '⦁', color = color(na), textcolor = swShow and swTerm == 'Short Term' ? swBullColor : color(na), style = label.style_label_down, size = textSize(swSwingsSZ), tooltip = str.tostring(field_10[1], format.mintick))
    stHigh.lastLabel
...
var array<liq> b_liq_B = array.new<liq>(1, liq.new(box(na), box(na), box(na), false, false, line(na), line(na)))
var array<liq> b_liq_S = array.new<liq>(1, liq.new(box(na), box(na), box(na), false, false, line(na), line(na)))
    ...
    if true
        ...
        if count > 2
            ...
            if st_B == getB.bx.get_left()
                ...
                b_liq_B.unshift(liq.new(box.new(st_B, math.avg(minP, maxP) + atr / liqMar, bar.index + 10, math.avg(minP, maxP) - atr / liqMar, bgcolor = color(na), border_color = color(na)), box.new(na, na, na, na, bgcolor = color(na), border_color = color(na)), box.new(st_B, st_P, bar.index + 10, st_P, text = 'Buyside liquidity', text_size = size.tiny, text_halign = text.align_left, text_valign = text.align_bottom, text_color = color.new(cLIQ_B, 25), bgcolor = color(na), border_color = color(na)), false, false, line.new(st_B, st_P, bar.index - 1, st_P, color = color.new(cLIQ_B, 0)), line.new(bar.index - 1, st_P, na, st_P, color = color.new(cLIQ_B, 0), style = line.style_dotted)))
    ...
    if true
        ...
        if count > 2
            ...
            if st_B == getB.bx.get_left()
                ...
                b_liq_S.unshift(liq.new(box.new(st_B, math.avg(minP, maxP) + atr / liqMar, bar.index + 10, math.avg(minP, maxP) - atr / liqMar, bgcolor = color(na), border_color = color(na)), box.new(na, na, na, na, bgcolor = color(na), border_color = color(na)), box.new(st_B, st_P, bar.index + 10, st_P, text = 'Sellside liquidity', text_size = size.tiny, text_halign = text.align_left, text_valign = text.align_top, text_color = color.new(cLIQ_S, 25), bgcolor = color(na), border_color = color(na)), false, false, line.new(st_B, st_P, bar.index - 1, st_P, color = color.new(cLIQ_S, 0)), line.new(bar.index - 1, st_P, na, st_P, color = color.new(cLIQ_S, 0), style = line.style_dotted)))
...
for i = 0 to b_liq_B.size() - 1 by 1
    x = b_liq_B.get(i)
...
for i = 0 to b_liq_S.size() - 1 by 1
    x = b_liq_S.get(i)
...
if lqVoid and dispPeriod
    ...
    field_13 = bar.close
    field_14 = bar.high
    bull = bar.low - field_11[2] > atr200 and bar.low > field_12[2] and field_13[1] > field_14[2]
    field_15 = bar.low
    field_16 = bar.low
    field_17 = bar.close
    field_18 = bar.low
    bear = field_15[2] - bar.high > atr200 and bar.high < field_16[2] and field_17[1] < field_18[2]

    if bull
        l = 13
        if bull[1]
            field_19 = bar.low
            st = math.abs(bar.low - field_19[1]) / l
            for i = 0 to l - 1 by 1
                field_20 = bar.low
                field_21 = bar.low
                b_liq_V.push(box.new(bar.index - 2, field_20[1] + i * st, bar.index, field_21[1] + (i + 1) * st, border_color = na, bgcolor = cLQV_B))
        else
            field_22 = bar.high
            st = math.abs(bar.low - field_22[2]) / l
            for i = 0 to l - 1 by 1
                if lqText and i == 0
                    field_23 = bar.high
                    field_24 = bar.high
                    array.push(b_liq_V, box.new(bar.index - 2, field_23[2] + i * st, bar.index, field_24[2] + (i + 1) * st, text = 'Liquidity Void   ', text_size = size.tiny, text_halign = text.align_right, text_valign = text.align_bottom, text_color = na, border_color = na, bgcolor = cLQV_B))
                else
                    field_25 = bar.high
                    field_26 = bar.high
                    array.push(b_liq_V, box.new(bar.index - 2, field_25[2] + i * st, bar.index, field_26[2] + (i + 1) * st, border_color = na, bgcolor = cLQV_B))

    if bear
        l = 13
        if bear[1]
            field_27 = bar.high
            st = math.abs(field_27[1] - bar.high) / l
            for i = 0 to l - 1 by 1
                array.push(b_liq_V, box.new(bar.index - 2, bar.high + i * st, bar.index, bar.high + (i + 1) * st, border_color = na, bgcolor = cLQV_S))
        else
            field_28 = bar.low
            st = math.abs(field_28[2] - bar.high) / l
            for i = 0 to l - 1 by 1
                if lqText and i == l - 1
...
if b_liq_V.size() > 0
    ...
    for bn = qt - 1 to 0 by 1
        if bn < b_liq_V.size()
            ...
            field_30 = bar.close
            field_31 = bar.close
            if math.sign(field_29[1] - ba) != math.sign(bar.close - ba) or math.sign(field_30[1] - ba) != math.sign(bar.low - ba) or math.sign(field_31[1] - ba) != math.sign(bar.high - ba)
                b_liq_V.remove(bn)
            else
...
if obbShow
    ...
    if bar.close > top.y and not top.crossed
        ...
        for i = 1 to bar.index - top.x - 1 by 1
    ...
    if bullish_ob.size() > 0
        for i = bullish_ob.size() - 1 to 0 by 1
    ...
    if bar.close < btm.y and not btm.crossed
        ...
        for i = 1 to bar.index - btm.x - 1 by 1
    ...
    if bearish_ob.size() > 0
        for i = bearish_ob.size() - 1 to 0 by 1
